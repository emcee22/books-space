...
export class BooksCarousel extends HTMLElement {
    ...
    // holds the interval so we can clearInterval
    ...
    createSlides(): void {
        ...
        this._slides.forEach(slide => {
            ...
            author.innerHTML = slide.authors && slide.authors[0] ? slide.authors[0].name : '-';
        ...
        this._slider.prepend(this._shadowRoot.querySelector('.slider ul li:last-child'));
        // autoplay slider
    ...
    slideInit() {
        ...
        this.slideWidth = this.getSlideWidth(sliderWidth, this.preferredSlidesPerPage);

        // set all slide width
        this._shadowRoot.querySelectorAll('.slider ul li').forEach((slide: any) => {
            slide.style.width = `${this.slideWidth}px`;
        });

        // get number of slides
        const slideCount = this._shadowRoot.querySelectorAll('.slider ul li').length;
    ...
    getSlideWidth(sliderWidth: number, preferredSlidesPerPage: number): number {
        ...
        if (calcSlideWidth > this.maximumSlideWidth && preferredSlidesPerPage > 1) {
            preferredSlidesPerPage += 1;
            return this.getSlideWidth(sliderWidth, preferredSlidesPerPage);
        }
    ...
    moveRight(event: any = null) {
        ...
        animation.onfinish = () => {
            this._slider.append(this._shadowRoot.querySelector('.slider ul li:first-child'));
        };
    }
    ...
    preloadImage(element: any) {
        const src: string = element.querySelector('[data-src]').getAttribute('data-src');
